...
def ObjectDetection(cap):
    ...
    if ret == True:
            ...
            BLUE_MIN = np.array([95,50,50])
            BLUE_MAX = np.array([105,255,255])

            GREEN_MIN = np.array([35,30,30])
            GREEN_MAX = np.array([50,255,255])

            YELLOW_MIN = np.array([55,30,30])
            YELLOW_MAX = np.array([65,255,255])
            ...
            for c in contours:
                ...
                if Extent > 0.7:
                    ...
                    if(x>(cap.get(3)/2)):
                         x = x - w
                    cv2.putText(canny_edge_green,('Green, Height:'+str(h)),(x+w+10,y+h),0,0.3,(150,255,0))
                    D_DetermanentG = 8000
                    Distance = round((D_DetermanentG/h),2)
                    cv2.putText(canny_edge_green,('Distance'+str(Distance)),(x+w+10,y-20),0,0.3,(150,255,0))
                    capwidth  = cap.get(3)
            ...
            for c in contoursb:
                ...
                if Extent > 0.7:
                    ...
                    cv2.putText(canny_edge_blue,('Oriantation'+OriantationStr),(x+w+10,y-10),0,0.3,(150,255,0))
                    D_DetermanentB = 4200
                    Distance = round((D_DetermanentB/h),2)
                    cv2.putText(canny_edge_blue,('Distance'+str(Distance)),(x+w+10,y-20),0,0.3,(150,255,0))
                    capwidth  = cap.get(3)
                    Bearing = (((x+(w/2))-(capwidth/2))/(7.25))
                    Bearing1 = round(Bearing,2)
            ...
            for c in contourso:
                 ...
                 if Extent > 0.7:
                     ...
                     cv2.putText(canny_edge_orange,('Orange, Height:'+str(h)),(x+w+10,y+h),0,0.3,(150,255,0))
                     D_DetermanentO = 2200
            ...
            for c in contourso:
                 ...
                 if Extent > 0.7:
                     ...
                     cv2.putText(canny_edge_yellow,('Yellow, Height:'+str(h)),(x+w+10,y+h),0,0.3,(150,255,0))
                     D_DetermanentO = 2200
                     Distance = round((D_DetermanentO/h),2)
                     cv2.putText(canny_edge_yellow,('Distance'+str(Distance)),(x+w+10,y-20),0,0.3,(150,255,0))
                     capwidth  = cap.get(3)
                     Bearing = ((x+(w/2))-(capwidth/2))/(7.25)
                     Bearing1 = round(Bearing,2)
